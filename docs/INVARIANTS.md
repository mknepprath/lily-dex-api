...
**1. Megas are published as forms.** `mega-forms-present`
...
**2. Every mega form is complete.** `mega-forms-complete`
...
**3. A form carries its species' dex number.** `forms-carry-base-dex`
...
**4. Dex entries are unique.** `dex-entries-unique`
A duplicate double-counts a species in collection totals, which are the app's
central number.

**5. Species ids align positionally with dex numbers.** `species-ids-aligned`
...
**6. Event titles are decoded.** `titles-decoded`
`Pokémon XP &amp; 2026 Worlds` shipped to users.

**7. No expired battle format is published.** `formats-not-expired`
...
**8. Format ids are unique.** `format-ids-unique`
...
**9. Every published format carries rankings.** `formats-have-rankings`
...
Or `npm run conformance`. CI runs it after the build, so a broken invariant fails
the deploy rather than reaching users.
